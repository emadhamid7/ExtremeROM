...
# Runs sed on a decompiled smali file only if it exists. Missing files are
# logged and skipped instead of hard-failing the build (common on stripped
# low-end bases that don't ship Knox/enterprise/carrier-specific classes).
SAFE_SED()
{
    local pattern="$1"
    local target="$2"
    if [[ -f "$target" ]]; then
        ...
        LOG "  - Skipping (not present in source): ${target#$APKTOOL_DIR/}"
    fi
}
...
if [[ "$SOURCE_PRODUCT_FIRST_API_LEVEL" != "$TARGET_PRODUCT_FIRST_API_LEVEL" ]]; then
    ...
    "
    for f in $FTP; do
        SAFE_SED "s/\"MAINLINE_API_LEVEL: $SOURCE_PRODUCT_FIRST_API_LEVEL\"/\"MAINLINE_API_LEVEL: $TARGET_PRODUCT_FIRST_API_LEVEL\"/g" "$APKTOOL_DIR/$f"
        SAFE_SED "s/\"$SOURCE_PRODUCT_FIRST_API_LEVEL\"/\"$TARGET_PRODUCT_FIRST_API_LEVEL\"/g" "$APKTOOL_DIR/$f"
    done
    LOG_STEP_OUT
fi
...
if [[ "$SOURCE_AUTO_BRIGHTNESS_TYPE" != "$TARGET_AUTO_BRIGHTNESS_TYPE" && "$TARGET_AUTO_BRIGHTNESS_TYPE" != "4" ]]; then
    ...
    "
    for f in $FTP; do
        SAFE_SED "s/\"$SOURCE_AUTO_BRIGHTNESS_TYPE\"/\"$TARGET_AUTO_BRIGHTNESS_TYPE\"/g" "$APKTOOL_DIR/$f"
    done
...
if [[ "$SOURCE_FP_SENSOR_CONFIG" != "$TARGET_FP_SENSOR_CONFIG" ]]; then
    ...
    "
    for f in $FTP; do
        SAFE_SED "s/$SOURCE_FP_SENSOR_CONFIG/$TARGET_FP_SENSOR_CONFIG/g" "$APKTOOL_DIR/$f"
    done
...
if [[ "$(GET_FP_SENSOR_TYPE "$TARGET_FP_SENSOR_CONFIG")" == "optical" ]]; then
    ...
    "
    for f in $FTP; do
        SAFE_SED "s/green_circle/ripple/g" "$APKTOOL_DIR/$f"
        SAFE_SED "s/white_circle/ripple/g" "$APKTOOL_DIR/$f"
    done
fi
...
if ! $SOURCE_HAS_HW_MDNIE; then
    if $TARGET_HAS_HW_MDNIE; then
        LOG "- Skipping HW mDNIe patches (disabled: patches removed)"
    fi
fi

if ! $SOURCE_MDNIE_SUPPORT_HDR_EFFECT; then
    if $TARGET_MDNIE_SUPPORT_HDR_EFFECT; then
        LOG "- Skipping mDNIe HDR effect patches (disabled: patch removed)"
    fi
fi
...
if [[ "$SOURCE_MDNIE_SUPPORTED_MODES" != "$TARGET_MDNIE_SUPPORTED_MODES" ]]; then
    ...
    "
    for f in $FTP; do
        SAFE_SED "s/\"$SOURCE_MDNIE_SUPPORTED_MODES\"/\"$TARGET_MDNIE_SUPPORTED_MODES\"/g" "$APKTOOL_DIR/$f"
    done
    LOG_STEP_OUT
fi
...
if [[ "$TARGET_HFR_SEAMLESS_BRT" == "none" && "$TARGET_HFR_SEAMLESS_LUX" == "none" ]]; then
    ...
    "
    for f in $FTP; do
        SAFE_SED "s/\"$SOURCE_HFR_SEAMLESS_BRT\"/\"$TARGET_HFR_SEAMLESS_BRT\"/g" "$APKTOOL_DIR/$f"
        SAFE_SED "s/\"$SOURCE_HFR_SEAMLESS_LUX\"/\"$TARGET_HFR_SEAMLESS_LUX\"/g" "$APKTOOL_DIR/$f"
    done
fi
...
if [[ "$SOURCE_HFR_MODE" != "$TARGET_HFR_MODE" ]]; then
    ...
    "
    for f in $FTP; do
        SAFE_SED "s/\"$SOURCE_HFR_MODE\"/\"$TARGET_HFR_MODE\"/g" "$APKTOOL_DIR/$f"
    done
    ...
    SAFE_SED "s/\"$SOURCE_HFR_MODE\"/\"$REPL\"/g" "$APKTOOL_DIR/system/framework/framework.jar/smali_classes6/com/samsung/android/rune/CoreRune.smali"
    LOG_STEP_OUT
fi
...
if [[ "$SOURCE_HFR_SUPPORTED_REFRESH_RATE" != "$TARGET_HFR_SUPPORTED_REFRESH_RATE" ]]; then
    ...
    for f in $FTP; do
        if [[ "$TARGET_HFR_SUPPORTED_REFRESH_RATE" != "none" ]]; then
            SAFE_SED "s/\"$SOURCE_HFR_SUPPORTED_REFRESH_RATE\"/\"$TARGET_HFR_SUPPORTED_REFRESH_RATE\"/g" "$APKTOOL_DIR/$f"
        else
            SAFE_SED "s/\"$SOURCE_HFR_SUPPORTED_REFRESH_RATE\"/\"\"/g" "$APKTOOL_DIR/$f"
        fi
    done
    LOG_STEP_OUT
fi
if [[ "$SOURCE_HFR_DEFAULT_REFRESH_RATE" != "$TARGET_HFR_DEFAULT_REFRESH_RATE" ]]; then
    ...
    "
    for f in $FTP; do
        SAFE_SED "s/\"$SOURCE_HFR_DEFAULT_REFRESH_RATE\"/\"$TARGET_HFR_DEFAULT_REFRESH_RATE\"/g" "$APKTOOL_DIR/$f"
    done
    LOG_STEP_OUT
fi
...
if [[ "$SOURCE_DVFS_CONFIG_NAME" != "$TARGET_DVFS_CONFIG_NAME" ]]; then
    ...
    "
    for f in $FTP; do
        SAFE_SED "s/\"$SOURCE_DVFS_CONFIG_NAME\"/\"$TARGET_DVFS_CONFIG_NAME\"/g" "$APKTOOL_DIR/$f"
    done
    LOG_STEP_OUT
fi
